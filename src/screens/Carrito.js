...
const Carrito = ({ navigation }) => {
  ...
  const getDetalleCarrito = async () => {
    try {
      const response = await fetch(`${ip}/lacteos_queso/api/services/public/pedido.php?action=readDetail`, {
        method: 'GET',
      });
      ...
      Alert.alert('Error', 'Ocurrió un error al listar el catalogo');
    }
  };
  ...
  const finalizarPedido = async () => {
    try {
      const response = await fetch(`${ip}/lacteos_queso/api/services/public/pedido.php?action=finishOrder`, {
        method: 'GET',
      });
      const data = await response.json();
      if (data.status) {
