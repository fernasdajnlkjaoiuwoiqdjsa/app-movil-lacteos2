...
/*
          ...
          [
            ...
            },
            {
              ...
              onPress: async () => {
                const formData = new FormData();
                formData.append('idDetalle', idDetalle);
                const response = await fetch(`${ip}/lacteos_queso/api/services/public/pedido.php?action=deleteDetail`, {
                  method: 'POST',
                  body: formData
                });
                const data = await response.json();
  ...
  return (
    ...
    <Text style={styles.itemText}>Nombre: {item.nombre_detalle}</Text>
    <Text style={styles.itemText}>Precio: ${item.precio_catpro}</Text>
    <Text style={styles.itemText}>Cantidad: {item.cantidad_catpro}</Text>
    <Text style={styles.itemText}>SubTotal: ${(parseFloat(item.cantidad_catpro)*parseFloat(item.precio_catpro)).toFixed(2)}</Text>

    <TouchableOpacity style={styles.modifyButton}
    onPress={()=>accionBotonDetalle(item.id_detalle, item.precio_catpro)}
    >
...
const styles = StyleSheet.create({
  ...
  modifyButton: {
    ...
    paddingVertical: 8,
    paddingHorizontal: 16,
    backgroundColor: '#2A8C4A', // Light brown color for modify button
    marginVertical: 4,
  },
  deleteButton: {
    borderWidth: 1,
    ...
    paddingVertical: 8,
    paddingHorizontal: 16,
    backgroundColor: '#2A8C4A', // Darker orange color for delete button
    marginVertical: 4,
  },
  buttonText: {
    textAlign: 'center',
